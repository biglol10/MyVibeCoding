...
공개 배포용으로 바로 실행 가능한 zip을 만들려면 Apple Developer 계정의 `Developer ID Application` 인증서와 notarization 설정이 필요합니다.

```bash
cd MyMacCalendar
MYMACCALENDAR_DEVELOPER_ID="Developer ID Application: Your Name (TEAMID1234)" \
MYMACCALENDAR_NOTARY_PROFILE="mymaccalendar-notary" \
...
```text
dist/MyMacCalendar-0.1.0-macOS.zip
```

Developer ID 인증서가 없는 개인용 배포는 개인 설치 zip으로 만들 수 있습니다.
...
```text
dist/MyMacCalendar-personal-mac.zip
```

개인용 zip은 다운로드 격리 속성 제거와 로컬 ad-hoc 서명을 수행하는 `Install MyMacCalendar.command`를 포함합니다. 압축 해제 후 앱을 직접 더블클릭하지 말고 설치 스크립트를 우클릭해서 여세요.
