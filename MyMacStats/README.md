...
현재 배포 파일은 개발/테스트용 빌드입니다. Apple Developer ID 서명과 공증이 없기 때문에, GitHub에서 다운로드한 zip을 바로 실행하면 macOS Gatekeeper가 "손상되었으므로 휴지통으로 이동" 경고를 표시할 수 있습니다. 처음 실행할 때는 아래 방법 중 하나를 사용하세요.
...
3. `열기`를 선택합니다.
4. 헬퍼가 `MyMacStats.app`의 다운로드 격리 속성을 제거한 뒤 앱을 실행합니다.
