...
개인 맥북에 설치하는 용도라면 Developer ID 공증 없이도 동봉된 설치 스크립트를 사용하면 됩니다. 앱을 직접 더블클릭하지 말고 `Install MyMacStats.command`를 실행하세요. 이 스크립트가 앱을 `/Applications`에 복사하고 다운로드 격리 속성을 제거한 뒤 실행합니다.

### 개인 맥북 설치 방법

Developer ID 인증서가 없는 개인용 zip은 설치 스크립트를 사용합니다.
